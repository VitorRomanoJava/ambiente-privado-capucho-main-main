// src/components/MockupGenerator.tsx
...
// 1. IMPORTAÇÕES ADICIONADAS PARA RESPONSIVIDADE
...
import {
  ...
  FlipHorizontal, FlipVertical, Settings // Ícone de configurações adicionado
} from "lucide-react";
...
// 2. PAINEL DE CONTROLES EXTRAÍDO PARA UM COMPONENTE
// Isso evita a duplicação de código entre a visão de desktop e o painel móvel.
const ControlPanel = ({
  selectedProduct, setSelectedProduct, setIsArtEditorOpen, directFileInputRef, finalArtUrl,
  globalSizeMultiplier, setGlobalSizeMultiplier, settings, handleSettingChange, setArtFlip,
  productColor, setProductColor, handleDownloadMockup, handleDownloadArt
}: any) => (
        ...
        <CardContent className="space-y-6 pt-2">
          ...
          <div className="space-y-2 pt-4 border-t border-muted">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" onClick={() => setArtFlip((f: any) => ({ ...f, horizontal: !f.horizontal }))}>
                <FlipHorizontal className="w-4 h-4 mr-2" /> Horizontal
              </Button>
              <Button variant="outline" onClick={() => setArtFlip((f: any) => ({ ...f, vertical: !f.vertical }))}>
...
const MockupGenerator = () => {
  ...
  const canvas3DRef = useRef<CanvasHandle>(null);
  ...
  // 3. ESTADO PARA CONTROLAR O PAINEL MOBILE
  ...
  // --- Funções de Lógica (handleDownload, handleDirectArtUpload, etc. sem alterações) ---
  const handleDownloadMockup = async () => {
    ...
    try {
      const canvas = await html2canvas(mockupArea, { backgroundColor: null, logging: false, useCORS: true });
      const link = document.createElement("a");
  ...
  const handleSaveArtFromEditor = (artDataUrl: string) => {
    ...
    setArtFlip({ horizontal: false, vertical: false });
    setIsArtEditorOpen(false); // Fecha o editor ao salvar
    toast({ title: "Arte Aplicada!", description: "Sua arte criada no editor foi aplicada ao produto." });
  };

  const commonViewerProps = {
    productColor: productColor, uploadedImage: finalArtUrl, customText: "", textColor: "", textFont: "", textSize: 0, 
    textureOffsetX: settings.textureOffsetX, imageScaleX: settings.imageScaleX * globalSizeMultiplier * (artFlip.horizontal ? -1 : 1),
    ...
    textScaleY: 0, textOffsetX: 0, textOffsetY: 0, textRotation: 0, selectedProduct: selectedProduct,
  };
  
  // Agrupando as props para o ControlPanel para facilitar a leitura
  const controlPanelProps = {
    selectedProduct, setSelectedProduct: (value: string) => { setSelectedProduct(value); setIsSheetOpen(false); },
    setIsArtEditorOpen, directFileInputRef, finalArtUrl, globalSizeMultiplier, setGlobalSizeMultiplier,
    settings, handleSettingChange, setArtFlip, productColor, setProductColor, handleDownloadMockup, handleDownloadArt
  };


  // 4. JSX PRINCIPAL REESTRUTURADO PARA RESPONSIVIDADE
  return (
    <>
      <ArtEditor 
      ...
      <section className="py-12 md:py-20 bg-gradient-subtle">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {/* BOTÃO PARA ABRIR CONTROLES NO MOBILE (visível apenas em telas pequenas) */}
            <div className="lg:hidden mb-4">
                <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                    <SheetTrigger asChild>
                        <Button variant="outline" className="w-full">
                            <Settings className="w-4 h-4 mr-2" />
                            Abrir Controles e Ajustes
                        </Button>
                    </SheetTrigger>
                    <SheetContent side="left" className="w-[350px] sm:w-[400px] p-4 overflow-y-auto">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              {/* PAINEL DE CONTROLES PARA DESKTOP (escondido em telas pequenas) */}
              <div className="hidden lg:block lg:col-span-1">
                <ControlPanel {...controlPanelProps} />
              </div>

              {/* VISUALIZADOR DE MOCKUP (ocupa a tela toda em mobile) */}
